...
survival_data <- trait_data %>%
  ...
  mutate(
    ...
    survival_rate = OriginalTraitValue / 100
  )
...
We can now continue the same analysis done prior for the rest of the traits, starting with the exploratory plots
...
We decided to use a briere function instead of a quadratic function due to the properties of the data
...
For Oocyst prevalence and intensity, we decided to combine them together 

```{r}
trait_data <- read_csv("~/Downloads/vec_traits_download_Wed Apr 30 2025 19_32_46 GMT-0400 (Eastern Daylight Time).csv", show_col_types = FALSE)
...
### JAGS Model for Prevalence (Quadratic)
...
### JAGS Model for Intensity (Log-Normal)
...
```
```{r}
summary(samples_prev)
```

```{r}
summary(samples_intensity)
```
